...
export async function POST(req: NextRequest) {
  ...
  const below = part.lessonPlan.parts.find((p) => p.order === part.order + 1);
  if (below) {
    await prisma.$transaction([
      prisma.lessonPlanPart.update({ where: { id: part.id }, data: { order: below.order } }),
      prisma.lessonPlanPart.update({ where: { id: below.id }, data: { order: part.order } }),
    ]);
  }

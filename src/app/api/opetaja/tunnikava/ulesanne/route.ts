...
export async function POST(req: NextRequest) {
  ...
  await prisma.taskUsage.upsert({
    where: { taskId_lessonPlanId: { taskId: task.id, lessonPlanId: entry.lessonPlan.id } },
    update: {},
    create: { taskId: task.id, lessonPlanId: entry.lessonPlan.id },
  });

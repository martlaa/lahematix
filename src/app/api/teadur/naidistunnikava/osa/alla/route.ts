...
export async function POST(req: NextRequest) {
  ...
  const below = part.sampleLessonPlan.parts.find((p) => p.order === part.order + 1);
  if (below) {
    await prisma.$transaction([
      prisma.sampleLessonPlanPart.update({ where: { id: part.id }, data: { order: below.order } }),
      prisma.sampleLessonPlanPart.update({ where: { id: below.id }, data: { order: part.order } }),
    ]);
  }
